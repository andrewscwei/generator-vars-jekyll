...
gulp.task('scripts', function()
{
  function bundle(bundler, output, next)
  {
    return bundler.bundle()
      ...
      .pipe(gulp.dest(config.paths.tmp+'/assets/js'));
  }

    return merge
    (
        gulp.src(config.paths.generated+'/assets/js/*.'+config.patterns.scripts)
        ...
        gulp.src([config.paths.generated+'/assets/vendor/**/*.'+config.patterns.scripts])
